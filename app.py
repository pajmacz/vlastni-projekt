...
def index():
	...
	if request.method == "POST":
		...
		spravne_heslo="Pavel Doležal"or "pavel doležal"
...
@app.route("/sýr", methods=["GET", "POST"])
def syr():
	odpoved = None
	current_route = request.path
	last_route = session.get("last_route", None)
	...
	if request.method == "POST":
		...
		if heslo==spravne_heslo:
			...
			pocet_pokusu += 1
			session["pocet_pokusu"] = pocet_pokusu
	return render_template("syr.html", odpoved=odpoved, pocet_pokusu=pocet_pokusu)
